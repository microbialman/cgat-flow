...
def main(argv=sys.argv):
    ...
    parser.add_argument(
        "-f", "--filter-method", dest="filter", type=str,
        choices=("unfinished", "running", "completed", "all"),
        help="apply filter to output")
    ...
    if args.sections:
        profile_sections = args.sections
    else:
    ...
    infile = iotools.openFile(args.logfile)
    ...
    for section in profile_sections:
        ...
        for objct, c in counts[section].items():
            ...
            # apply filters
            if args.filter in ("unfinished", "running") and c.running == 0:
                continue
